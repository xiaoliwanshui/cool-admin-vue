...
import { categoryGet } from "/$/video/utils/category";
const { service } = useCool();
const category_pidDict = ref<Array<{ label: string; value: string | number }>>([]);
onMounted(async () => {
	category_pidDict.value = await categoryGet({ parent_id: 0 });
});
...
const Upsert = useUpsert({
	items: [
		...
		{ label: "imd评分", prop: "imdb_score", component: { name: "el-rate" } },
		{
			label: "iimdID",
			prop: "imdb_score_id",
			component: { name: "el-input", props: { clearable: true } }
		},
		{ label: "豆瓣评分", prop: "douban_score", component: { name: "el-rate" } },
		{
			label: "豆瓣ID",
...
const Table = useTable({
	columns: [
		...
		{ label: "分类一级", prop: "category_name", dictColor: true, minWidth: 120 },
		{
			label: "分类二级",
			prop: "category_child_name",
			dictColor: true,
			minWidth: 120
		},
		{
